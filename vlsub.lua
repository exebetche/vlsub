...
local options = {
  ...
  translation = {
    ...
    int_feeling_lucky = "I'm feeling lucky type behavior",
...
function interface_config()
  ...
  dlg:add_label(
    lang["int_feeling_lucky"]..':', 1, 6, 0, 1)
  input_table['feelingLucky'] = dlg:add_dropdown(3, 6, 1, 1)
  ...
  input_table['feelingLucky']:add_value(
    lang["int_bool_"..tostring(openSub.option.feelingLucky)], 1)
  input_table['feelingLucky']:add_value(
    lang["int_bool_"..tostring(not openSub.option.feelingLucky)], 2)
...
-- Apply user config selection to local config
  ...
  if input_table["removeTag"]:get_value() == 2 then
    ...
    if input_table["feelingLucky"]:get_value() == 2 then
    openSub.option.feelingLucky = not openSub.option.feelingLucky
  end
...
function feeling_lucky()
  searchHash() -- first search by hash
  if openSub.itemStore == "0" then -- if no result search by name
    searchIMBD()
  end
...
function download_subtitles(feelingLucky)
