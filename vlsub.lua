...
openSub = {
  ...
  request = function(methodName)
    ...
    local header = {
      "POST "..path.." HTTP/"..openSub.conf.versionHTTP,
      "Host: "..host,
  ...
  -- Get video file path, name, extension from input uri
    ...
    if not item then
      ...
      file.uri = item:uri()
      file.name = string.gsub(item:name(), "\"", "")
...
function get(url)
  ...
  local header = {
    "GET "..path.." HTTP/"..openSub.conf.versionHTTP,
    "Host: "..host,
    "User-Agent: "..openSub.conf.userAgentHTTP,
    "",
    ""
